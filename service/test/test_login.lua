...
local function connect_gate(account)
    local gate_fd = socket.open("127.0.0.1", gate_port)
    local msg = account.acc.."@"..(account.subid or 0)
    skynet.error("connecttttttttttttt gate:"..gate_fd)
    skynet.error("connecttttttttttttt gate port:"..gate_port)
...
local function auth_login(account)
    -- local fd = socket.open("127.0.0.1", login_port)
    -- skynet.sleep(500)
    -- skynet.error(login_port)
    -- local response
    --
